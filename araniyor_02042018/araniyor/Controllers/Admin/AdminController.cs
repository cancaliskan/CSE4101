...
namespace araniyor.Controllers.Admin
{
    ...
    public class AdminController : Controller
    {
        ...
        [ValidateAntiForgeryToken]
        [ValidateInput(false)]
        public ActionResult EditHakkimizda(string HakkimizdaResimTemp,[Bind(Include = "ID,Hakkimizda,Misyon,Vizyon,Adres,Telefon,EPosta,Facebook,Twitter,LinkedIn,KullaniciSozlesmesi")] AboutUs aboutUs
            ...
            , HttpPostedFileBase SliderResim3, HttpPostedFileBase SliderResim4)
        {
           ...
           // var tempKayit = db.AboutUs.Where(x => x.ID == aboutUs.ID).SingleOrDefault();
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    aboutUs.HakkimizdaResim = new byte[HakkimizdaResim.ContentLength];
                    HakkimizdaResim.InputStream.Read(aboutUs.HakkimizdaResim, 0, HakkimizdaResim.ContentLength);
                }
                else
                {
                    aboutUs.HakkimizdaResim =  Encoding.UTF8.GetBytes(HakkimizdaResimTemp);
                }
