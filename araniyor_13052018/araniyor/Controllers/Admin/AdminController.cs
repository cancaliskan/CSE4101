    ...
    public class AdminController : Controller
    {
        ...
        public ActionResult ForgetPassword(string eMail)
        {
            ...
            else
            {
                ...
                TempData["mesaj"] = "Bilgileriniz gönderildi. Lütfen posta kutunuzu kontrol ediniz.";
        ...
        public ActionResult CreateSubBusinessCategory([Bind(Include = "businessSubategoryID,businessCategoryID,businessSubategory1")] businessSubategory businessSubategory)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    var temp = db.businessSubategory.Where(a => a.businessSubategory1 == businessSubategory.businessSubategory1
                                                           && a.businessCategoryID == businessSubategory.businessCategoryID).FirstOrDefault();
                    if (temp == null)
                    {
        ...
        [ValidateAntiForgeryToken]
        [ControlLogin]
        // public ActionResult EditUser([Bind(Include = "userID,businessCategoryID,businessSubategoryID,username,password,eMail,name,surname,steward,city,district,birthday,phone,description,score,numberOfComments,numberOfViews,active,gender,experience,admin")] Users users, HttpPostedFileBase fileup)
        public ActionResult EditUser(int userID, int businessCategoryID, int businessSubategoryID, string username, string password, string eMail, string name
                                     , string surname, bool steward, string city, string district, DateTime? birthday, string phone, string description, float? score, int? numberOfComments, int? numberOfViews, bool active
                                     , string gender, int? experience, bool admin, HttpPostedFileBase fileup)
        {
                ...
                if (steward == true)
                {
                    ...
                    && birthday != null && description != null && gender != null && experience != null))
                    {
                        if (ModelState.IsValid)
                        {
                            ...
                            user.phone = phone;
                            user.score = score;
                            user.numberOfComments = numberOfComments;
                            user.numberOfViews = numberOfViews;
                            user.active = active;
                            user.gender = gender;
        ...
        public ActionResult CreateMesaj([Bind(Include = "messageID,senderID,receiverID,message,date,conversationID")] messages messages)
        {
            ...
            {
                messages.conversationID ="s"+ Convert.ToString(messages.senderID) +"r"+ Convert.ToString(messages.receiverID);
                if (ModelState.IsValid)
                {
